...
def weight_func(param_cond, K):
    '''
    To generate the weights of the components given the conditiontial parameters. Not sure what is wrong.
    '''
    D_cond = len(param_cond)
    np.random.seed(9)
    matr = np.random.permutation(K*D_cond*10)[:K*D_cond].reshape(K, D_cond) / (K*D_cond*5)
    weights0 = np.dot(matr, (param_cond)**1.2)
...
def means_func(param_cond, K, D):
    ...
    matr -= matr.mean()
    means0 = np.dot(matr, (param_cond)**1.4)
